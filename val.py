...
from helper_functions.logger import setup_logger
from models import create_model
import numpy as np

parser = argparse.ArgumentParser(description='PyTorch TreeSatAI Validation')
# parser.add_argument('data', metavar='DIR', help='path to dataset')
parser.add_argument('--backbone-name', default='tresnet_l')
parser.add_argument('--ckpt-path', default='./TRresNet_L_448_86.6.pth', type=str)
...
def main():
    ...
    args.model_path = False

    # setup logger
    logger = setup_logger(output=args.output, color=False, name="treesatai")
...
def validate_multi(val_loader, model, args, logger):
    ...
    logger.info(
        '--------------------------------------------------------------------')
    logger.info(' * P_C {:.2f} R_C {:.2f} F_C {:.2f} P_O {:.2f} R_O {:.2f} F_O {:.2f}'
          .format(mean_p_c, mean_r_c, mean_f_c, p_o, r_o, f_o))

    mAP_score = mAP(torch.cat(targets).numpy(), torch.cat(preds).numpy())
    logger.info("mAP score: {}".format(mAP_score))
